...
/*let's find the min and max GPU's for each gpu_name category*/
with cte_priceRank as(
select *, rank() over (partition by gpu_name order by currentPrice desc) as priceRank from #finalResultTableProcess )
select gpu_name, gpu_category_id from cte_priceRank group by gpu_name, gpu_category_id having priceRank = 1
;

select distinct gpu_name, gpu_category_id from #finalResultTableProcess order by gpu_category_id;
